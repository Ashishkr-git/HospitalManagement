...
const Tabs = React.forwardRef<HTMLDivElement, TabsProps>(
  (
    ...
    const registerTabsList = React.useCallback(
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn(
        `relative inline-flex h-auto items-center justify-center rounded-full bg-muted text-primary`,
        className
      )}
      {...props}
    >
      <motion.div
        layout
        className="absolute top-0 left-0 h-full rounded-full tabs-bg-indicator bg-gradient-tabs"
        style={{
          ...indicatorStyle,
          position: "absolute",
...
>(({ className, value, ...props }, ref) => {
  const { value: selectedValue, updateIndicator } =
    React.useContext(TabsContext);
  const isActive = selectedValue === value;
